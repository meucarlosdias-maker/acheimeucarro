...
export async function getVeiculoPorSlug(slug) {
  ...
  const { data, error } = await supabase
    .from("veiculos")
    .select(`
      id, versao, ano_fab, ano_modelo, km, preco, preco_promocional, combustivel, cambio, cor, cor_interna,
      portas, lugares, categoria, tracao, tipo_direcao, situacao,
      descricao, video_url, slug, created_at, caracteristicas,
      marca:marcas ( nome ),
      modelo:modelos ( nome ),
...
export async function getVeiculosPorModelo(modeloNome, excludeSlug, limit = 6) {
  const hoje = new Date().toISOString().split("T")[0];

  const { data } = await supabase
    .from("veiculos")
    .select(`
      id, versao, ano_fab, ano_modelo, km, preco, slug,
      marca:marcas ( nome ),
      modelo:modelos!inner ( nome ),
      revenda:revendas ( nome, slug, plano_fim, cidade:cidades ( nome, slug ) ),
      fotos:veiculo_fotos ( url, ordem )
    `)
    .eq("status", "ativo")
    .eq("modelo.nome", modeloNome)
